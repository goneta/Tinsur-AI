...
export function QuoteWizard({ initialClientId, onQuoteCreated, initialVehicleId, onExit }: QuoteWizardProps = {}) {
    ...
    const processMatchResult = useCallback((result: any) => {
        if (isPublicMarketplace) {
            ...
            const flattened = companies.flatMap((company: any) => company.policies.map((policy: any) => ({
                ...policy,
            ...
            const recommendedCompany = companies.find((c: any) => c.recommended_id);
        ...
        if (result.status === "success" && result.data && result.data.length > 0) {
            const annotated = result.data.map((policy: any) => ({
                ...policy,
