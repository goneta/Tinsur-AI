        ...
        <div className="space-y-6 p-8 pt-6">
            ...
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                <div className="flex items-center space-x-2">
                    ...
                    <div>
                        ...
                        <p className="text-muted-foreground">{t('Policy Details & History', 'Policy Details & History')}</p>
                    </div>
                </div>
            ...
            <div className="grid gap-6 md:grid-cols-3">
                ...
                <div className="md:col-span-2 space-y-6">
                    <Card>
                        <CardHeader>
                            <div className="flex justify-between items-start">
                                <div>
                                    <CardTitle>{t('Policy Information', 'Policy Information')}</CardTitle>
                                    <CardDescription>{t('Coverage and premium details', 'Coverage and premium details')}</CardDescription>
                                </div>
                        ...
                        <CardContent className="grid gap-6 sm:grid-cols-2">
                            <div className="space-y-1">
                                <span className="text-sm font-medium text-muted-foreground">{t('Start Date', 'Start Date')}</span>
                                <p className="font-medium">{format(new Date(policy.start_date), 'PPP')}</p>
                            </div>
                            <div className="space-y-1">
                                <span className="text-sm font-medium text-muted-foreground">{t('End Date', 'End Date')}</span>
                                <p className="font-medium">{format(new Date(policy.end_date), 'PPP')}</p>
                            </div>
                            <div className="space-y-1">
                                <span className="text-sm font-medium text-muted-foreground">{t('Premium Amount', 'Premium Amount')}</span>
                                <p className="text-lg font-bold">{formatCurrency(policy.premium_amount)}</p>
                            </div>
                            <div className="space-y-1">
                                <span className="text-sm font-medium text-muted-foreground">{t('Frequency', 'Frequency')}</span>
                                <p className="capitalize font-medium">{t(policy.premium_frequency, policy.premium_frequency)}</p>
                            </div>
                            <div className="space-y-1">
                                <span className="text-sm font-medium text-muted-foreground">{t('Coverage Amount', 'Coverage Amount')}</span>
                                <p className="font-medium">{policy.coverage_amount ? formatCurrency(policy.coverage_amount) : 'N/A'}</p>
                            </div>
                            <div className="space-y-1">
                                <span className="text-sm font-medium text-muted-foreground">{t('Created By', 'Created By')}</span>
                    ...
                    <Card>
                        <CardHeader>
                            ...
                            <CardDescription>{t('Recent transactions for this policy', 'Recent transactions for this policy')}</CardDescription>
                        </CardHeader>
                        <CardContent>
                            {payments.length === 0 ? (
                                <div className="text-center py-6 text-muted-foreground">
                                    {t('No payments recorded yet', 'No payments recorded yet')}
                                </div>
                            ) : (
                                <Table>
                                    <TableHeader>
                                    ...
                                    <TableBody>
                                        {payments.map((p) => (
                                            <TableRow key={p.id}>
                                                ...
                                                <TableCell>{formatDate(p.created_at, language === 'fr' ? 'fr-FR' : 'en-US')}</TableCell>
                                                <TableCell>{formatCurrency(p.amount, p.currency)}</TableCell>
                                                <TableCell className="capitalize">{p.payment_method.replace('_', ' ')}</TableCell>
                                                <TableCell>{getPaymentStatusBadge(t(p.status))}</TableCell>
                ...
                <div className="space-y-6">
                    <Card>
                        <CardHeader>
                            <CardTitle>{t('Client Details', 'Client Details')}</CardTitle>
                        </CardHeader>
                        <CardContent className="space-y-4">
                            ...
                            <Button variant="outline" className="w-full" onClick={() => router.push(`/dashboard/clients/${policy.client_id}`)}>
                                {t('View Client Profile', 'View Client Profile')}
                            </Button>
                        </CardContent>
                    </Card>
                    ...
                    <Card>
                        ...
                        <CardContent className="space-y-2">
                            ...
                            >
                                ...
                                {t('Policy Agreement.pdf', 'Policy Agreement.pdf')}
                            </Button>
                            <Button
                                variant="outline"
                                className="w-full justify-start"
                    ...
                    <Card className="border-2 border-primary/20">
                        <CardHeader className="pb-2">
                            <CardTitle className="flex items-center gap-2">
                                ...
                                {t('Policy Verification', 'Policy Verification')}
                            </CardTitle>
                            <CardDescription>{t('Authentication for third-parties', 'Authentication for third-parties')}</CardDescription>
                        </CardHeader>
                        <CardContent className="space-y-4 pt-4">
                            {!policy.qr_code_data ? (
                                <div className="text-center py-4 space-y-4">
                                    <p className="text-xs text-muted-foreground italic">{t('No verification token generated for this policy.', 'No verification token generated for this policy.')}</p>
                                    <Button
                                        onClick={async () => {
                                            try {
                                        ...
                                        className="w-full"
                                    >
                                        {t('Generate Token', 'Generate Token')}
                                    </Button>
                                </div>
                            ) : (
                                <div className="flex flex-col items-center gap-4 py-2">
                                    ...
                                    <div className="text-center">
                                        <p className="text-[10px] font-mono text-muted-foreground uppercase tracking-widest mb-1">{t('Secure Token', 'Secure Token')}</p>
                                    ...
                                    >
                                        ...
                                        {t('Preview Public Page', 'Preview Public Page')}
                                    </Button>
                                </div>
                            )}
                        </CardContent>
                    ...
                    <Card>
                        <CardHeader className="flex flex-row items-center justify-between pb-2">
                            <CardTitle className="text-sm font-bold">{t('Co-insurance', 'Co-insurance')}</CardTitle>
                        ...
                        <CardContent className="space-y-4 pt-2">
                            {shares.length === 0 ? (
                                <p className="text-xs text-muted-foreground italic text-center py-2">{t('No participants added.', 'No participants added.')}</p>
                            ) : (
                                <div className="space-y-3">
                                    {shares.map(s => (
                                        <div key={s.id} className="flex items-center justify-between bg-gray-50 p-2 rounded text-xs border border-gray-100">
                                            <div className="flex flex-col">
                                                ...
                                                <span className="text-[10px] text-muted-foreground">{t('Fee', 'Fee')}: {s.fee_percentage}%</span>
                                            </div>
                                    ...
                                    <div className="pt-2 border-t flex justify-between items-center px-1">
                                        <span className="text-[10px] font-bold text-muted-foreground uppercase">{t('Total Shared', 'Total Shared')}</span>
