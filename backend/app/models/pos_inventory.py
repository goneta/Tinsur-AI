...
class POSInventory(Base):
    ...
    pos_location_id = Column(GUID(), ForeignKey("pos_locations.id", ondelete="CASCADE"))
...
class POSInventoryTransaction(Base):
    ...
    id = Column(GUID(), primary_key=True, default=uuid.uuid4)
    inventory_id = Column(GUID(), ForeignKey("pos_inventory.id", ondelete="CASCADE"))
    ...
    reference_id = Column(GUID(), nullable=True) # e.g. Policy ID, or Restock Order ID
