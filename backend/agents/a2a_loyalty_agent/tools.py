...
@tool
def get_loyalty_points(client_id: str) -> str:
    """
    ...
    db = SessionLocal()
    try:
        loyalty = db.query(LoyaltyPoint).filter(LoyaltyPoint.client_id == uuid.UUID(client_id)).first()
        if not loyalty:
            return f"No loyalty account found for client {client_id}."
...
@tool
def redeem_loyalty_points(client_id: str, points_to_redeem: int) -> str:
    """
    ...
    try:
        loyalty = db.query(LoyaltyPoint).filter(LoyaltyPoint.client_id == uuid.UUID(client_id)).first()
        if not loyalty:
            return f"Error: No loyalty account found for client {client_id}."
